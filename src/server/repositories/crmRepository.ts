...
export interface CrmRepository {
  ...
  addUser(input: CreateUserInput): Promise<{ user: User; temporaryPassword: string }>;
...
export class InMemoryCrmRepository implements CrmRepository {
  ...
  async addUser(input: CreateUserInput) {
    ...
    // Generate a strong random temporary password; the inviter receives it once
    const temporaryPassword = randomBytes(18).toString('base64url');
    const defaultHash = await hashPassword(temporaryPassword, this.passwordPepper);
    this.passwordHashes.set(user.id, defaultHash);
    return { user: clone(user), temporaryPassword };
  }
  ...
  async mergeContacts(sourceId: string, targetId: string, finalValues: UpdateContactInput) {
    ...
    // Tenant isolation: both contacts must belong to the current org
    if (!this.checkOrg(this.contacts[sourceIdx]) || !this.checkOrg(this.contacts[targetIdx])) {
      return null;
    }
    ...
    // Whitelist allowed columns (same as updateContact / Postgres mergeContacts)
    const ALLOWED_COLS = new Set([
      'first_name', 'last_name', 'email', 'phone', 'title', 'linkedin_url',
      'owner_id', 'account_id', 'tags', 'custom_fields',
    ]);
    const safeValues: UpdateContactInput = {};
    for (const [key, val] of Object.entries(finalValues)) {
      if (ALLOWED_COLS.has(key)) (safeValues as Record<string, unknown>)[key] = val;
    }

    // Update target with safe final values, remove source
    this.contacts[targetIdx] = { ...this.contacts[targetIdx], ...safeValues };
  ...
  async listPipelines() {
    return this.filterByOrg(clone(this.pipelines));
  }

  async listStages() {
    return this.filterByOrg(clone(this.stages));
  }
  ...
  async updatePipeline(id: string, input: { name?: string; is_default?: boolean; is_archived?: boolean }): Promise<Pipeline | null> {
    ...
    if (idx === -1 || !this.checkOrg(this.pipelines[idx])) return null;
    if (input.is_default) {
  ...
  async deletePipeline(id: string): Promise<boolean> {
    const idx = this.pipelines.findIndex(p => p.id === id);
    if (idx === -1 || !this.checkOrg(this.pipelines[idx])) return false;
  ...
  async updateStage(id: string, input: { name?: string; probability?: number; order?: number; type?: 'open' | 'won' | 'lost' }): Promise<Stage | null> {
    ...
    if (idx === -1 || !this.checkOrg(this.stages[idx])) return null;
  ...
  async deleteStage(id: string): Promise<boolean> {
    const idx = this.stages.findIndex(s => s.id === id);
    if (idx === -1 || !this.checkOrg(this.stages[idx])) return false;
